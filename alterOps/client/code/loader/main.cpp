...
int CALLBACK WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd)
{
	...
	{
		MessageBox(NULL, "Could not find " EXE_NAME " in the directory this executable is located. Make sure it actually exists, and try again.", "secretSchemes", MB_ICONERROR | MB_OK);
		return 1;
	}
	...
	{
		MessageBox(NULL, "Could not find " DLL_NAME " in the directory this executable is located. Make sure it actually exists, and try again.", "secretSchemes", MB_ICONERROR | MB_OK);
		return 1;
	}
	...
	{
		MessageBox(NULL, "Could not start the process.", "secretSchemes", MB_ICONERROR | MB_OK);
		return 2;
	}
	...
	if (!memory)
	{
		MessageBox(NULL, "Could not allocate memory in the process.", "secretSchemes", MB_ICONERROR | MB_OK);
		return 3;
	}
	...
	if (!thread)
	{
		MessageBox(NULL, "Could not start remote thread.", "secretSchemes", MB_ICONERROR | MB_OK);
		return 3;
	}
